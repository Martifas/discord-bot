...
describe('DELETE', () => {
  ...
  describe('?id=:id', () => {
    ...
    it('should actually remove the template', async () => {
      const [template] = await createTemplates([fakeTemplate()])
      await supertest(app).delete(`/templates?id=${template.id}`).expect(200)
      await supertest(app).get(`/templates?id=${template.id}`).expect(404)
    })
  })
})
